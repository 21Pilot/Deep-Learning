...
def generate_model():
    ...
    if Path('.convnet_improved_model.json').is_file():
        sys.stdout.write('Loading existing model\n\n')
        sys.stdout.flush()

        with open('.convnet_improved_model.json') as file:
            model = keras.models.model_from_json(json.load(file))
            file.close()

        # likewise for model weight, if exists load from saved state
        if Path('.convnet_improved_weights.h5').is_file():
            model.load_weights('.convnet_improved_weights.h5')
    ...
    with open('.convnet_improved_model.json', 'w') as outfile:
...
def train(model, X_train, y_train, X_test, y_test):
    ...
    while epoch_count < epoch:
        ...
        sys.stdout.flush()
        model.save_weights('.convnet_improved_weights.h5')
